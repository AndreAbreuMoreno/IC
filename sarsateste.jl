...
function testaAlgoritmo()
    ...
    dados_teste = open("dados/teste.csv", "r")
    test = open("saida/result_teste$(iteracao).txt", "w")
    hist = open("saida/historico$(iteracao).txt", "w")
    hist2 = open("saida/historico_acumulado$(iteracao).txt", "w")

    cabecalho = readline(dados_teste)
    linet3 = split(readline(dados_teste),"\t")
    linet3 = parse.(Float32,linet3[3:8])
    linet2 = split(readline(dados_teste),"\t")
    linet2 = parse.(Float32,linet2[3:8])
    linet1 = split(readline(dados_teste),"\t")
    linet1 = parse.(Float32,linet1[3:8])
    linet0= split(readline(dados_teste),"\t")
    linet0= parse.(Float32,linet0[3:8])
