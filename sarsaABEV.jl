...
function criaQ(iteracao)
      if iteracao == 0
            ...
            chave = open("ABEV/dict/chave$(iteracao).txt", "r")
            valor = open("ABEV/dict/valor$(iteracao).txt", "r")
            dic = open("ABEV/dict/dicionario.txt", "w")
            keys= []
            vals = []
            while !eof(chave)
...
function testaAlgoritmo()
      """ Testa algoritmo"""
    estado_teste = zeros(Int64,10)
    proximo_estado_teste = zeros(Int64,10)
    dados_teste = open("ABEV/dados/ABEV3teste.csv", "r")
    test = open("ABEV/saida/result_teste$(iteracao).txt", "w")
    hist = open("ABEV/saida/historico$(iteracao).txt", "w")
    hist2 = open("ABEV/saida/historico_acumulado$(iteracao).txt", "w")

    cabecalho = readuntil(dados_teste,"%")
    linet3 = split(readuntil(dados_teste,"%"),"\t")
    linet3 = parse.(Float32,linet3[4:9])
    linet2 = split(readuntil(dados_teste,"%"),"\t")
    linet2 = parse.(Float32,linet2[4:9])
    linet1 = split(readuntil(dados_teste,"%"),"\t")
    linet1 = parse.(Float32,linet1[4:9])
    linet0= split(readuntil(dados_teste,"%"),"\t")
    linet0= parse.(Float32,linet0[4:9])
    primeiroEstado(estado_teste, linet0, linet1, linet2, linet3)
    premio = 0
    historico = []
    historico_acumulado = []
    while !eof(dados_teste)
          # global linet1
          # global linet0
          # global dados_teste
          # global estado_teste
          # global premio
          # global proximo_estado_teste
          # global historico
          # global historico_acumulado
          key = calculaKey(estado_teste)
          action = melhorAcao(test, key)
          println(test, "key: ", key)
          if key[3] == 0
                println(test, "1 ", Q_e_a[key[1]]," 2 ", Q_e_a[key[2]], " action: ", action%10, conjuntoAcao[action%10])
          else println(test, "1 ", Q_e_a[key[1]]," 2 ", Q_e_a[key[2]]," 3 ", Q_e_a[key[3]]," action: ", action%10, conjuntoAcao[action%10])
          end
          linet2 = linet1
          linet1 = linet0
          linet0= split(readuntil(dados_teste,"%"),"\t")
          linet0= parse.(Float32,linet0[4:9])
          r = calculaRecompensa(proximo_estado_teste, estado_teste, action, linet0)
          append!(historico, r)
          premio = premio + r
          append!(historico_acumulado, premio)
          println(test, "premio: ", premio)
          proximo_estado_teste = executaAcao(test, estado_teste, action,linet1, linet2)
          estado_teste = proximo_estado_teste


    end

    println(test, " premio acumulada: ", premio)
...
it = open("ABEV/dict/iteracao.txt", "r")
...
while iteracao < qtd_iteracao
      ...
      global dados = open("ABEV/dados/ABEV3training.csv", "r")
      result = open("ABEV/saida/resultado$(n).txt", "w")
      ...
      linhat3 = parse.(Float32,linhat3[4:9])
      linhat2 = split(readuntil(dados,"%"),"\t")
      linhat2 = parse.(Float32,linhat2[4:9])
      linhat1 = split(readuntil(dados,"%"),"\t")
      linhat1 = parse.(Float32,linhat1[4:9])
      linhat0= split(readuntil(dados,"%"),"\t")
      linhat0= parse.(Float32,linhat0[4:9])
      ...
      if iteracao%5000 == 0
            ...
            chave = open("ABEV/dict/chave$(iteracao).txt", "w")
            valor = open("ABEV/dict/valor$(iteracao).txt", "w")

            for (k,v) in Q_e_a
                      println(chave, k)
                      println(valor, v)
            end
            it = open("ABEV/dict/iteracao.txt", "w")
            println(it, iteracao)
...
estado = zeros(Int64,10)
proximo_estado = zeros(Int64,10)
dados = open("ABEV/dados/ABEV3teste.csv", "r")
test = open("ABEV/saida/result_teste.txt", "w")
hist = open("ABEV/saida/historico.txt", "w")
hist2 = open("ABEV/saida/historico_acumulado.txt", "w")
global date = []
...
data = open("ABEV/saida/data.txt", "w")
println(data, date)
